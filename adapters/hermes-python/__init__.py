...
# ── Global singleton: one shared bridge client per process ──────────
_shared_bridge_lock = threading.Lock()
_shared_bridge: MemosBridgeClient | None = None


def _invalidate_shared_bridge() -> None:
    """Invalidate the shared bridge singleton.

    Called when the TCP daemon is known to have been restarted and the
    old client's socket connection is stale. The next call to
    _get_shared_bridge() will create a fresh client.
    """
    global _shared_bridge
    with _shared_bridge_lock:
        if _shared_bridge is not None:
            logger.info("MemOS: invalidating shared bridge singleton")
            with contextlib.suppress(Exception):
                _shared_bridge.close()
            _shared_bridge = None


def _get_shared_bridge() -> MemosBridgeClient:
    """Return (creating if needed) the single shared MemosBridgeClient.

    All MemTensorProvider instances share this one client over TCP.
    This prevents each session from opening its own TCP connection
    and leaking file descriptors.
    """
    global _shared_bridge
    if _shared_bridge is not None:
        # Quick sanity check — if the transport is dead, recreate
        try:
            _shared_bridge.request("core.health", {}, timeout=2)
            return _shared_bridge
        except Exception:
            logger.warning("MemOS: shared bridge client stale, recreating")
            with _shared_bridge_lock:
                if _shared_bridge is not None:
                    with contextlib.suppress(Exception):
                        _shared_bridge.close()
    ...
    with _shared_bridge_lock:
        if _shared_bridge is not None:
            return _shared_bridge
        _shared_bridge = MemosBridgeClient()
        # Wait for daemon to be ready
        import time as _time
        for attempt in range(20):
            try:
                _shared_bridge.request("core.health", {}, timeout=5)
                logger.info("MemOS: shared bridge client ready (singleton)")
                break
            except Exception:
                if attempt == 19:
                    logger.error("MemOS: shared bridge client failed to connect to daemon after 10s")
                _time.sleep(0.5)
        return _shared_bridge
...
class MemTensorProvider(MemoryProvider):
    ...
    def __init__(self) -> None:
        # Use the shared singleton — do NOT create per-instance bridge.
        # We lazily get it in initialize() after TCP daemon is ready.
        ...
        # Session lifecycle guard — prevents deferred turn.end from firing
        # after the episode has been closed in on_session_end().
    ...
    def initialize(self, session_id: str, **kwargs: Any) -> None:  # type: ignore[override]
        ...
        logger.info(
            "MemOS MemTensorProvider.initialize() — session_id=%s platform=%s agent=%s hermes_home=%s",
            session_id, self._platform, self._agent_identity, self._hermes_home,
        )

        # Derive memos-plugin path from hermes_home: ~/.hermes/profiles/nova → ~/.hermes/profiles/nova/memos-plugin
        # Tell daemon_manager so bridge_client injects MEMOS_HOME env var.
        memos_plugin_dir = str(Path(self._hermes_home) / "memos-plugin")
        daemon_manager.set_memos_home(memos_plugin_dir)
        logger.info("MemOS: memos_home=%s", memos_plugin_dir)
        ...
        # Start the shared TCP daemon so all sessions connect via TCP
        # instead of each spawning their own stdio subprocess.
        try:
            daemon_manager.start_tcp_daemon(memos_plugin_dir)
        except Exception as err:
            logger.warning("MemOS: failed to start TCP daemon — %s (falling back to stdio)", err)
        try:
    ...
    def queue_prefetch(self, query: str, *, session_id: str = "") -> None:  # type: ignore[override]
        ...
        def _run() -> None:
            ...
            # Guard: only flush deferred turn.end if the session is still active.
            # If on_session_end() already closed the episode, skip the write to
            # avoid "episode is not open" errors.
            if pending and self._bridge and self._session_active:
                try:
    ...
    def on_session_end(self, messages: list[dict[str, Any]]) -> None:  # type: ignore[override]
        ...
        if pending:
            with contextlib.suppress(Exception):
                self._turn_end(*pending)
        # Mark session as inactive BEFORE closing the episode so any
        # concurrent queue_prefetch thread won't attempt turn.end on a
        # closed episode.
    ...
    def shutdown(self) -> None:  # type: ignore[override]
        if self._prefetch_thread and self._prefetch_thread.is_alive():
            self._prefetch_thread.join(timeout=5.0)
        # Do NOT close self._bridge — it's a shared singleton used by other
        # sessions. Just clear the reference so this instance stops using it.
        self._bridge = None
        with contextlib.suppress(Exception):
            daemon_manager.shutdown_bridge()
