...
export async function runL2(
  ...
  {
    const t0 = Date.now();
    associations = await associateTraces(eligibleTraces, {
      repos,
...

  {
    ...
    for (const bucket of ready) {
      ...
      const dup = await findExistingMatch(traces, repos, config.minSimilarity);
      if (dup) {
        inductions.push({
          signature: bucket.signature,
          policyId: dup.id,
...
async function findExistingMatch(
  ...
  for (const t of traces) {
    ...
    if (!vec) continue;
    const hits = await repos.policies.searchByVector(vec, 5, {
