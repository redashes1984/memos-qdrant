...
export interface ReposInitOptions {
...
export function makeRepos(db: StorageDb, opts?: ReposInitOptions): Repos {
  const qdrantOpts = opts
    ? { qdrant: opts.qdrant, vectorBackend: opts.vectorBackend }
    : undefined;
  return {
    ...
    policies: makePoliciesRepo(db, qdrantOpts),
    sessions: makeSessionsRepo(db),
    skills: makeSkillsRepo(db, qdrantOpts),
    traces: makeTracesRepo(db, qdrantOpts),
    worldModel: makeWorldModelRepo(db, qdrantOpts),
  };
}
