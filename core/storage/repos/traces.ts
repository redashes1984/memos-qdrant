...
/** Factory options for the traces repo. */
export interface TracesRepoOptions {
  qdrant?: QdrantStore | null;
...
export function makeTracesRepo(db: StorageDb, opts?: TracesRepoOptions) {
  ...
  return {
     ...
     *
     * When `vectorBackend == 'qdrant'`, delegates to the remote Qdrant
     * ANN index; otherwise falls back to SQLite brute-force scan.
     * Returns hits with the same `VectorHit<string, TraceSearchMeta>` shape.
     */
    ...
    ): Promise<Array<VectorHit<string, TraceSearchMeta>>> {
      // ── Qdrant path ────────────────────────────────────────────────
      if (qdrant && vectorBackend === "qdrant") {
        const kind = opts.kind ?? "summary";
        const suffix = `traces_${kind}`;
        ...
        return hits.map((h) => ({
          ...
          score: h.score,
          meta: {
            ts: Number(h.payload?.ts) || 0,
            priority: Number(h.payload?.priority) || 0,
            value: Number(h.payload?.value) || 0,
            episode_id: String(h.payload?.episode_id || ""),
            session_id: String(h.payload?.session_id || ""),
            tags_json: h.payload?.tags_json as string | undefined,
            error_signatures_json: h.payload?.error_signatures_json as string | undefined,
          },
        }));
      }

      // ── SQLite brute-force path (original) ─────────────────────────
      const kind = opts.kind ?? "summary";
