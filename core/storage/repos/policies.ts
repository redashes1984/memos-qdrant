...
export function makePoliciesRepo(db: StorageDb, opts?: TracesRepoOptions) {
      ...
      if (qdrant && vectorBackend === "qdrant") {
        ...
        return hits.map((h) => ({
          ...
          meta: {
            ...
            status: (h.payload?.status as PolicyRow["status"]) ?? "candidate",
            support: Number(h.payload?.support) || 0,
